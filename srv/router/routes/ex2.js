...
module.exports = () => {
	...
	app.get("/example1", (req, res) => {
		...
		if(req.authInfo && !req.authInfo.checkScope(scope)) {
			return res.type("text/plain").status(403).send("Forbiddden");
		}
		
		let client = req.db;
		client.prepare(
			`SELECT SESSION_USER, CURRENT_SCHEMA
				FROM "DUMMY"`, (err, statement) => {
					if(err) {
						return res.type("text/plain").status(500).send(`ERROR: ${err.toString()}`);
					}	
		...
		try{
			...
			const statement = await db.preparePromisified(
				`SELECT TOP 1 CURRENT_USER, SESSION_USER, SESSION_CONTEXT('XS_APPLICATIONUSER') as APPLICATION_USER, 
				 SESSION_CONTEXT('APPLICATION') as APPLICATION, SESSION_CONTEXT('XS_COUNTRY') as XSCOUNTRY, SESSION_CONTEXT('XS_CLIENT') as XS_CLIENT
				 FROM DUMMY`);
